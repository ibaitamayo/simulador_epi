...
server <- function(input, output, session) {
  ...
  capture_current_scenario_config <- function() {
    list(
      app_file = "app_epidemiologic_v16_scenario_lab_step6_presets_guided_exploration.R",
  ...
  output$deployment_checklist_table <- renderTable({
    data.frame(
      ...
      Status = c(
        ifelse(!is.null(COUNTRY_AGE_DISTRIBUTION_WPP$data), "available", "fallback or missing"),
  ...
  output$evaluation_report <- renderText({
    ...
    lines <- c(
      ...
      paste0("app_file=app_epidemiologic_v16_scenario_lab_step6_presets_guided_exploration.R"),
      paste0("active_scenario_mode=", isolate(input$active_scenario_mode)),
