  ...
  sidebarLayout(
    sidebarPanel(
      ...
      div(class = "fixed-ref-box",
        ...
        tags$p(class = "small-note", "The comparator is loaded as a reference and is not recalculated when the simulation is run. In automatic mode, the app tries to use a comparator with the same SIRD/SEIRD structure as the active simulation. The SEIRD file must be included in the deployment environment.")
      ),
      ...
      conditionalPanel(
        ...
        numericInput(
          "exposed_period_days",
          "Days from exposure to active phase:",
          value = 4,
          min = 1,
          max = 14,
          step = 1
...
server <- function(input, output, session) {
  ...
  capture_current_scenario_config <- function() {
    list(
      app_file = "app_epidemiologic_v16_scenario_lab_step4_english_ui.R",
  ...
  output$evaluation_report <- renderText({
    ...
    lines <- c(
      ...
      paste0("app_file=app_epidemiologic_v16_scenario_lab_step4_english_ui.R"),
      paste0("active_scenario_mode=", isolate(input$active_scenario_mode)),
  ...
  output$selected_scenario_card <- renderUI({
    ...
    if (is.null(item) || is.null(item$summary)) {
      return(tags$div(class = "scenario-card", tags$p("No scenario selected.")))
    }
    s <- item$summary
    comparator <- NULL
    if (!is.null(sim_data$covid$data)) {
      ...
      comparator <- scenario_summary_from_results("Comparator", cfg, covid_like)
    }
    tags$div(class = "scenario-card",
      h4(s$Name),
      tags$p(tags$b("Model: "), s$Model, " - ", tags$b("Initial country: "), s$StartCountry),
      tags$div(class = "scenario-kpi-grid",
        ...
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Active peak"), tags$div(class = "scenario-kpi-value", format(round(s$PeakActive), big.mark = ","))),
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Peak E"), tags$div(class = "scenario-kpi-value", ifelse(is.na(s$PeakExposed), "NA", format(round(s$PeakExposed), big.mark = ",")))),
      ...
      tags$p(tags$b("Structural warning: "), s$StructureWarning),
  ...
  output$scenario_library_table <- renderTable({
    ...
    df[, c("Name", "Model", "StartCountry", "Days", "R0", "PeakDay", "PeakActive", "FinalDeaths", "CountriesReached", "Comparator", "StructureWarning"), drop = FALSE]
  }, digits = 2)
  ...
  output$scenario_comparison_table <- renderTable({
    ...
    out[, c("Name", "Model", "StartCountry", "PeakDay", "PeakActive", "PeakActivePercentWorld", "PeakExposedDay", "PeakExposed", "FinalDeaths", "CountriesReached", "DeltaPeakDayVsComparator", "DeltaPeakActiveVsComparator", "DeltaFinalDeathsVsComparator"), drop = FALSE]
  }, digits = 2)
