...
variant_calibration_values <- function(x) {
  switch(
    x,
    "conservative" = list(observable_fraction = 0.001, establishment_multiplier = 0.6, label = "Conservadora"),
...
ui <- fluidPage(
  ...
  titlePanel(
    ...
    h4("Simulación configurable frente a una referencia COVID - herramienta R/Shiny para docencia, planificación y visualización", style = "color: #7F8C8D;")
  ),
    ...
    sidebarPanel(
      ...
      selectInput(
        ...
        choices = c(
          ...
          "Modo técnico completo" = "complete"
        ),
        selected = "guided"
      ),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        checkboxInput("show_technical_validation_options", "Mostrar opciones de validación técnica", value = FALSE)
      ),
      div(class = "mode-box",
        conditionalPanel(
        ...
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
          tags$b("Modo técnico completo"),
          tags$p(class = "advanced-note", "Muestra todos los controles técnicos: calibración, movilidad, semilla, robustez, parámetros dinámicos y diagnósticos.")
        )
      ),
      div(class = "workflow-guide-box",
        conditionalPanel(
          ...
          tags$ol(
            ...
            tags$li("Define la expansión inicial, la fase expuesta, la duración activa y la mortalidad media."),
        ...
        conditionalPanel(
          ...
          tags$ol(
            tags$li("Configura motor, edad, semilla y movilidad cuando se necesite reproducibilidad técnica."),
            tags$li("Define el escenario de patógeno y los parámetros dinámicos."),
            ...
            tags$li("Usa los informes copiables para comparar ejecuciones.")
          )
        )
      ),
      div(class = "fixed-ref-box",
        ...
        conditionalPanel(
          ...
          selectInput(
            ...
            choices = c(
              "Automático según el modelo activo" = "auto",
      ...
      div(class = "simulation-action-box",
        ...
        tags$div(class = "secondary-action-row",
          actionButton("reset_defaults", "Restablecer valores por defecto", icon = icon("rotate-left"), class = "btn btn-default"),
          downloadButton("download_csv", "Descargar resultados CSV", class = "btn btn-default")
        ),
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
          tags$div(style = "margin-top: 8px;",
            actionButton("apply_covid_values", "Aplicar referencia Omicron a la simulación", icon = icon("flask"), class = "btn btn-default", width = "100%")
          )
        ),
      ...
      conditionalPanel(
        ...
        selectInput(
          ...
          "Tipo de modelo:",
          choices = c(
            "Modelo agregado básico" = "basic",
            "Modelo ajustado por edad" = "age_adjusted"
          ),
          selected = "age_adjusted"
        ),
        helpText("Opciones de validación técnica. El modo básico usa automáticamente el motor ajustado por edad con mortalidad diferencial por grupo etario."),
        selectInput(
          "disease_compartment_model",
          "Estructura de compartimentos:",
          choices = c(
            "SEIRD: incluye fase expuesta previa" = "SEIRD",
        ...
        conditionalPanel(
          ...
          selectInput(
            "age_parameter_mode",
            "Modo de parámetros por edad:",
            choices = c(
              "Technical validation: same mortality in all groups" = "neutral",
              "Recomendado: mortalidad diferencial por edad" = "differential_cfr"
            ),
            selected = "differential_cfr"
          ),
          numericInput("age_cfr_scale", "Ajuste global de mortalidad por edad:", value = 1, min = 0, max = 10, step = 0.1),
          selectInput(
            "age_distribution_mode",
            "Distribución por edad:",
            choices = c(
              "Pirámides de edad específicas por país" = "country_specific",
              "Average global pyramid" = "global_average"
            ),
            selected = "country_specific"
          ),
          helpText("El modo específico por país lee country_age_distribution_wpp2024_6groups.rds/csv cuando está disponible. La pirámide global queda como opción de validación técnica.")
        ),
        hr()
      ),
      ...
      conditionalPanel(
        ...
        selectInput(
          ...
          "Modo avanzado de selección del país inicial:",
          choices = c(
            "Random on each run" = "random_each_run",
            "Stable random with seed" = "seeded_stable",
            "País seleccionado manualmente" = "manual"
          ),
          selected = "manual"
        )
      ),
      ...
      conditionalPanel(
        condition = "input.active_scenario_mode == 'guided'",
        actionButton("random_start_country", "Elegir país aleatorio", icon = icon("dice"), class = "btn btn-outline-primary btn-sm"),
      ...
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        helpText("En modo completo puedes usar selección manual, aleatoria o aleatoria estable con semilla visible.")
      ),
      ...
      conditionalPanel(
        ...
        selectInput(
          "air_travel_scenario",
          "Escenario de tráfico aéreo internacional:",
          choices = c(
            "Tráfico restringido (25% de referencia)" = "restricted",
            "Tráfico reducido (60% de referencia)" = "reduced",
            "Tráfico de referencia (100%)" = "reference",
            "Tráfico alto / vacacional (150%)" = "high"
          ),
          selected = "reference"
        ),
        numericInput("import_establishment_probability", "Modificador de oportunidad de establecimiento importado:", value = 1.00, min = 0, max = 3, step = 0.05),
        helpText("Multiplicador aplicado a la oportunidad agregada de que una entrada internacional genere transmisión local. 1,0 no añade atenuación adicional.")
      ),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete' && input.starting_country_mode != 'random_each_run'",
        textInput("stochastic_seed", "Semilla para reproducibilidad (vacío = aleatorio):", value = ""),
      ...
      numericInput(
        "exposed_period_days",
        "Fase expuesta antes de transmisión activa (días):",
        value = 4,
        min = 1,
        max = 14,
        step = 1
      ),
      helpText("SEIRD usa E = fase expuesta: personas ya incorporadas al proceso de infección modelizado, pero todavía no en fase activa/transmisora. El valor por defecto de 4 días es un supuesto docente/de planificación tipo Omicron; prueba 3-5 días para análisis de sensibilidad."),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        selectInput(
          "infectiousness_profile",
          "Perfil de transmisión durante ese periodo:",
          choices = c(
            "Pico intermedio" = "mid",
            "Pico temprano" = "early",
            "Pico tardío" = "late",
            "Distribución plana" = "flat"
          ),
          selected = "mid"
        ),
        helpText("Define cómo se distribuye la contribución a la transmisión durante el periodo activo. El modo básico usa el valor por defecto.")
      ),
      ...
      conditionalPanel(
        ...
        selectInput(
          ...
          "Módulo dinámico de variantes:",
          choices = c(
            "Desactivado" = "off",
            "Activo para esta simulación" = "active"
          ),
          selected = "active"
        ),
        helpText("El modo completo muestra parámetros técnicos del módulo dinámico. Solo se aplica a la simulación configurable."),
        conditionalPanel(
          condition = "input.evolution_module_mode == 'active'",
          numericInput("mutation_rate_per_replication", "Tasa macroefectiva de oportunidad de reemplazo:", value = 5e-7, min = 1e-9, max = 1e-3, step = 1e-7),
          helpText("Parámetro agregado del escenario. No es una tasa molecular por base; escala oportunidades de reemplazo poblacional dentro del modelo."),
          numericInput("effective_mutation_targets", "Multiplicador de oportunidad poblacional:", value = 30, min = 1, max = 300, step = 1),
          selectInput(
            "variant_emergence_calibration",
            "Calibración macroscópica de aparición de variantes:",
            choices = c(
              "Conservadora" = "conservative",
              "Referencia macro para virus RNA" = "reference",
              "High incidence / pandemic scale" = "high"
            ),
            selected = "reference"
          ),
          numericInput("maximum_variant_R0_multiplier", "Multiplicador máximo de R0/Rt frente a variante inicial:", value = 1.65, min = 1.0, max = 3.0, step = 0.05),
          helpText("Este límite evita inflación acumulada no realista. El R0/Rt efectivo se pondera por la frecuencia de variantes. Es un límite de escenario, no una predicción.")
        )
      ),
      hr(),
      ...
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        textInput("containment_label", "Etiqueta del periodo de medidas:", value = "Periodo de contención")
      ),
      ...
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        radioButtons("global_plot_scale", "Escala de los gráficos:", choices = c("Personas" = "persons", "Porcentaje de la población mundial" = "percent"), selected = "persons")
      ),
      hr(),

      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        h4("Representatividad y calibración", style = "color: #1F618D; font-weight: bold;"),
        checkboxInput("enable_calibration_targets", "Mostrar diagnósticos de calibración", value = TRUE),
        numericInput("calibration_population", "Población de referencia para calibración:", value = WORLD_POPULATION, min = 1, step = 1000000),
        numericInput("observed_positive_cumulative", "Positivos acumulados observados (número absoluto):", value = NA, min = 0, step = 100000),
        numericInput("case_detection_fraction", "Fracción de detección especificada por el usuario (0-1):", value = 0.20, min = 0.0001, max = 1.0, step = 0.01),
        checkboxInput("estimate_detection_from_serology", "Estimar fracción de detección usando seroprevalencia y positivos", value = TRUE),
        numericInput("observed_positive_day", "Día de observación para positivos acumulados:", value = 60, min = 0, max = 1095, step = 1),
        ...
        numericInput("observed_seroprevalence_percent", "Seroprevalencia observada (% de población de referencia):", value = NA, min = 0, max = 100, step = 0.1),
        numericInput("observed_seroprevalence_day", "Día de observación para seroprevalencia:", value = 90, min = 0, max = 1095, step = 1),
        ...
        checkboxInput("enable_day0_date_estimation", "Estimar fecha calendario del día simulado 0", value = FALSE),
            ...
            fluidRow(
              ...
              column(6,
                ...
                tags$ul(
                  ...
                  tags$li("Opciones de validación técnica")
                )
              )
            )
          ),
...
server <- function(input, output, session) {
  ...
  capture_current_scenario_config <- function() {
    list(
      app_file = "app_epidemiologic_v16_scenario_lab_step6b_basic_exposed_parameter_ES_fix2.R",
  ...
  dynamic_preset_values <- function(preset) {
    ...
    if (identical(preset, "low")) {
      return(list(enabled = TRUE, scenario = "low", scenario_label = "Baja presión de oportunidad", rate = 1e-7, targets = 20, calibration = "conservative", max_multiplier = 1.10, adaptive_saturation = TRUE, saturation_exponent = 1.0, interpretation = "Low: expected to generate few or no established replacement events in many runs; adaptive-space saturation is active."))
    }
    if (identical(preset, "high")) {
      return(list(enabled = TRUE, scenario = "high", scenario_label = "Alta presión de oportunidad", rate = 2e-6, targets = 50, calibration = "high", max_multiplier = 1.65, adaptive_saturation = TRUE, saturation_exponent = 1.0, interpretation = "High: stress-test scenario with more population-level opportunities; adaptive-space saturation limits marginal gains near the cap."))
    }
    list(enabled = TRUE, scenario = "reference", scenario_label = "Presión de oportunidad de referencia", rate = 5e-7, targets = 30, calibration = "reference", max_multiplier = 1.35, adaptive_saturation = TRUE, saturation_exponent = 1.0, interpretation = "Reference: exploratory moderate population-level opportunity pressure; adaptive-space saturation is active.")
  }
  ...
  output$covid_reference_card <- renderUI({
    ...
    comparator_items <- list(
      ...
      c("Modelo", ref_model),
      c("E: fase expuesta", ifelse(identical(ref_model, "SEIRD"), paste0(ref_exposed_period, " días hasta fase activa"), "No aplica en SIRD")),
      c("Pico E", ifelse(is.na(ref_peak_exposed), "No disponible", paste0(ref_peak_exposed, " personas; día ", ref_peak_exposed_day))),
      c("Referencia", "COVID-19 Omicron"),
      c("País inicial", "Sudáfrica"),
      c("R0", "4.25"),
      c("Ventana activa", "5 días"),
  ...
  output$logic_availability_table <- renderTable({
    data.frame(
      Component = c(
        ...
        "Opciones de validación técnica"
      ),
      Guided_basic_mode = c(
        "Yes",
        "Yes",
  ...
  output$evaluation_report <- renderText({
    ...
    lines <- c(
      ...
      paste0("app_file=app_epidemiologic_v16_scenario_lab_step6b_basic_exposed_parameter_ES_fix2.R"),
      paste0("active_scenario_mode=", isolate(input$active_scenario_mode)),
  ...
  current_containment_countries_for_day <- function(selected_day) {
    schedule <- tryCatch(
      make_containment_schedule(
        ...
        custom_transmission_reduction = input$containment_transmission_reduction,
        custom_mobility_reduction = input$containment_mobility_reduction,
        label = ifelse(is.null(input$containment_label), "Periodo de contención", input$containment_label),
        geographic_scope = input$containment_geographic_scope,
        affected_continents = input$containment_affected_continents,
        affected_countries = input$containment_affected_countries
      ),
  ...
  update_country_map <- function(map_id, history, selected_day, value_label, palette_colors, popup_label, empty_message, boost_visibility = TRUE, use_choropleth = TRUE) {
    ...
    if (polygons_available && requireNamespace("sf", quietly = TRUE)) {
      ...
      polygon_popup <- if (isTRUE(map_interactive)) {
        ~paste0(
          ifelse(IsStartingCountry, "<b>País inicial</b><br>", ""),
      ...
      if (nrow(russia_marker_data) > 0 && is.finite(russia_marker_data$lng[1]) && is.finite(russia_marker_data$lat[1])) {
        russia_popup <- if (isTRUE(map_interactive)) {
          paste0(
            ifelse(russia_marker_data$IsStartingCountry[1], "<b>País inicial</b><br>", ""),
    ...
    marker_data <- data.frame(name = names(values), Value = as.numeric(values), stringsAsFactors = FALSE) %>%
      ...
      mutate(
        ...
        PopupPrefix = paste0(ifelse(IsStartingCountry, "<b>País inicial</b><br>", ""), ifelse(IsContainmentCountry, "<b>Containment active here</b><br>", ""))
      )
