...
make_containment_preset_table <- function() {
  data.frame(
    ...
    label = c("None", "Leve", "Moderada", "Intensa", "Custom"),
...
format_containment_schedule_for_display <- function(containment_schedule) {
  if (is.null(containment_schedule)) return(data.frame(Metric = "Containment", Value = "No disponible"))
  data.frame(
    Metric = c(
      "Enabled",
      "Preset",
...
calculate_horizon_diagnostics <- function(sim_result, mortality_rate = NA_real_, world_population = WORLD_POPULATION) {
  if (is.null(sim_result) || is.null(sim_result$data)) {
    return(data.frame(Metric = "Horizon diagnostic", Value = "No disponible", stringsAsFactors = FALSE))
  }
  d <- sim_result$data
...
ui <- fluidPage(
  tags$head(
    ...
    tags$script(HTML("
      ...
      Shiny.addCustomMessageHandler('updateVisibleTabs', function(x) {
        ...
        // Spanish UI labels. Visibility is label-based, so translated tabs must be
        // referenced with their translated titles.
        setTabVisible('Dinámica de variantes', dynamicOn);
        setTabVisible('Calibración y representatividad', advanced);
        setTabVisible('Medidas de contención', advanced);
        setTabVisible('Ajuste por edad y coste', true);
        setTabVisible('Edad y coste', true);
      });
    "))
  ),

  titlePanel(
    h2("Simulador epidemiológico poblacional con comparador COVID"),
  ...
  sidebarLayout(
    sidebarPanel(
      width = 4,
      h4("Parámetros de simulación", style = "color: #34495E; font-weight: bold;"),
      selectInput(
        "active_scenario_mode",
        "Nivel de detalle:",
        choices = c(
          "Modo básico guiado" = "guided",
      ...
      div(class = "mode-box",
        conditionalPanel(
          ...
          tags$b("Modo básico guiado"),
          tags$p(class = "advanced-note", "Muestra solo los controles principales del escenario. La semilla, calibración, tráfico aéreo, robustez y controles técnicos quedan en modo completo.")
        ),
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
      ...
      div(class = "workflow-guide-box",
        conditionalPanel(
          condition = "input.active_scenario_mode == 'guided'",
          tags$b("Orden sugerido en modo básico"),
          tags$ol(
            tags$li("Elige el país inicial o usa el botón de país aleatorio."),
            tags$li("Define la expansión inicial, duración activa y mortalidad media."),
            tags$li("Elige si se incluyen variantes con ventaja y medidas de contención."),
            tags$li("Define el horizonte temporal y ejecuta el escenario."),
            tags$li("Explora el mapa, las curvas y los resultados frente al comparador.")
          )
        ),
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
          tags$b("Orden sugerido en modo completo"),
          tags$ol(
            ...
            tags$li("Activa calibración o robustez solo si vas a interpretar los diagnósticos."),
            tags$li("Ejecuta el escenario y revisa mapas, curvas, informes y avisos."),
            tags$li("Use copyable reports to compare runs.")
          )
        )
      ),
      div(class = "fixed-ref-box",
        h4("Comparador COVID", style = "color: #27AE60; font-weight: bold;"),
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
          selectInput(
            ...
            choices = c(
              ...
              "Omicron básico SIRD RDS" = "basic",
              "Omicron ajustado por edad SIRD RDS" = "age_adjusted",
              "Omicron ajustado por edad SEIRD RDS" = "age_adjusted_seird"
            ),
            selected = "auto"
          )
        ),
        uiOutput("covid_reference_card"),
        tags$p(class = "small-note", "Este comparador representa un escenario teórico SEIRD con efecto de la edad aplicado al modelo y tráfico internacional de pasajeros entre países. Describe la trayectoria plausible estimada por el modelo si la primera variante hubiese sido Omicron y no se hubiesen aplicado medidas de contención en ningún momento.")
      ),
      div(class = "simulation-action-box",
        tags$div(class = "primary-run-button",
          actionButton("run_simulation", "Ejecutar simulación", icon = icon("play"), class = "btn btn-danger btn-lg")
        ),
        checkboxInput("auto_play_after_run", "Reproducir la animación automáticamente al terminar", value = TRUE),
        tags$div(class = "secondary-action-row",
          ...
          downloadButton("download_csv", "Descargar resultados CSV", class = "btn btn-default")
        ),
        conditionalPanel(
          condition = "input.active_scenario_mode == 'complete'",
        ...
        tags$p(class = "small-note", "Ejecuta el cálculo y muestra los mapas del escenario."),
        tags$details(class = "status-details",
          tags$summary("Estado de la última simulación"),
          verbatimTextOutput("computation_status")
        )
      ),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        h4("Características del modelo", style = "color: #34495E; font-weight: bold;"),
        selectInput(
          "model_structure",
          "Model type:",
          choices = c(
        ...
        selectInput(
          ...
          choices = c(
            ...
            "SIRD: transición directa a fase activa" = "SIRD"
          ),
          selected = "SEIRD"
        ),
        numericInput(
          "exposed_period_days",
          "Días desde exposición hasta fase activa:",
          value = 4,
          min = 1,
          max = 14,
          step = 1
        ),
        helpText("SEIRD adds E = exposed phase: people already incorporated into the modelled infection process, but not yet in the active/transmitting phase. Basic mode uses SEIRD with 4 days from exposure to active phase, compatible with Omicron incubation/latency estimates around 3-4 days."),
        conditionalPanel(
      ...
      h4("País donde se origina la infección", style = "color: #2E86C1; font-weight: bold;"),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        selectInput(
      ...
      selectInput("starting_country", "País donde se origina la infección:", choices = COUNTRIES_LIST, selected = sample(COUNTRIES_LIST, 1)),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'guided'",
        actionButton("random_start_country", "Choose random country", icon = icon("dice"), class = "btn btn-outline-primary btn-sm"),
        helpText("En modo básico puedes elegir el país manualmente o usar este botón para seleccionarlo al azar. La semilla permanece oculta, pero se guarda internamente para comparaciones y exportación.")
      ),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        helpText("In complete mode you can use manual, random, or stable random selection with visible seed.")
      ),
      numericInput("initial_infected", "Número inicial de casos:", value = 5, min = 1, max = 100000, step = 1),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        selectInput(
          "air_travel_scenario",
      ...
      h4("Patógeno a simular", style = "color: #E74C3C; font-weight: bold;"),
      numericInput("R0", "Capacidad inicial de expansión (R0/Rt):", value = 5.0, min = 0.5, max = 15.0, step = 0.1),
      helpText("Valor inicial de expansión agregada. Valores más altos implican expansión más rápida cuando no se aplican medidas reductoras."),
      numericInput("infectious_period_days", "Duración de la fase activa (días):", value = 20, min = 2, max = 60, step = 1),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        selectInput(
      ...
      numericInput("mortality_rate", "Mortalidad media (%):", value = 1.0, min = 0, max = 100, step = 0.1),
      helpText("Media objetivo entre casos resueltos. La app la distribuye por edad usando la estructura etaria de cada país. Valores muy altos se tratan como escenarios extremos."),
      hr(),

      h4("Aparición de variantes con ventaja", style = "color: #9B59B6; font-weight: bold;"),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'guided'",
        selectInput(
          "dynamic_module_scenario",
          "Aparición de variantes con ventaja:",
          choices = c(
            "Desactivado" = "off",
            "Low" = "low",
        ...
        helpText("Controla la aparición agregada de variantes con ventaja poblacional. Es estocástico: una sola ejecución no garantiza orden estricto entre bajo, intermedio y alto.")
      ),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
        selectInput(
      ...
      h4("Medidas de contención", style = "color: #7D3C98; font-weight: bold;"),
      selectInput(
        "containment_preset",
      ...
      selectInput(
        ...
        choices = c(
          "Global: todos los países" = "global",
      ...
      numericInput("containment_start_day", "Día de inicio de las medidas:", value = 210, min = 0, max = 1095, step = 1),
      numericInput("containment_end_day", "Día final de las medidas:", value = 240, min = 0, max = 1095, step = 1),
      conditionalPanel(
      ...
      helpText("Definiciones: leve = reducción parcial de transmisión y movilidad. Moderada = paquete intermedio. Intensa = reducción temporal de alta intensidad. Las medidas se aplican solo a la simulación configurable, no al comparador."),
      hr(),

      h4("Horizonte de simulación", style = "color: #E74C3C; font-weight: bold;"),
      numericInput("simulation_days", "Días simulados:", value = 365, min = 30, max = 1095, step = 30),
      conditionalPanel(
        condition = "input.active_scenario_mode == 'complete'",
      ...
      h4("Supuestos explícitos del modelo", style = "font-weight: bold;"),
    ...
    mainPanel(
      tabsetPanel(id = "main_tabs", type = "tabs",
        tabPanel("Mapa mundial",
          h4("Conectividad inicial antes de simular"),
          div(class = "map-preview-box",
            checkboxInput("show_connectivity_preview", "Mostrar conectividad inicial del país seleccionado", value = TRUE),
            conditionalPanel(
              condition = "input.show_connectivity_preview == true",
              tags$p(class = "small-note", "Muestra las 15 conexiones principales del país inicial. El ancho de línea representa el peso relativo de movilidad agregada usado por el modelo."),
              leafletOutput("connectivity_preview_map", height = "340px")
            )
          ),
          h4("Evolución geográfica durante la simulación"),
          div(
            style = "padding: 10px; margin-bottom: 10px; background: #F8F9F9; border-left: 4px solid #3498DB;",
            sliderInput("time_slider", "Día de simulación:", min = 0, max = 365, value = 0, step = 1),
            actionButton("play_animation", "Iniciar animación"),
            actionButton("pause_animation", "Pausar animación")
          ),
          div(id = "map_panel_anchor",
            fluidRow(
              column(6, h4("Activos por país"), leafletOutput("world_map_infected", height = "520px")),
              column(6, h4("Fallecimientos acumulados por país"), leafletOutput("world_map_deaths", height = "520px"))
            )
          )
        ),
        ...
        tabPanel("Laboratorio de escenarios",
          div(class = "scenario-lab-box",
            h4("Guardar y comparar escenarios"),
            ...
            selectInput(
              ...
              "Partir de un escenario predefinido:",
              choices = c(
                "Mantener configuración actual" = "none",
                "Línea base sin contención" = "no_containment",
                "Contención moderada temprana" = "early_moderate",
                "Contención moderada tardía" = "late_moderate",
                "Contención intensa" = "strong_containment",
                "Alta presión de reemplazo" = "high_replacement",
                "Alta movilidad sin contención" = "high_mobility"
              ),
              selected = "none"
            ),
            actionButton("apply_scenario_preset_template", "Aplicar predefinido", icon = icon("wand-magic-sparkles"), class = "btn btn-default"),
            tags$p(class = "scenario-preset-note", "Los escenarios predefinidos son puntos de partida para la exploración. Cambian los controles visibles, pero no alteran el motor del modelo."),
            textInput("scenario_name", "Nombre del escenario:", value = "Scenario A"),
            selectInput(
              "scenario_tag",
              "Etiqueta del escenario:",
              choices = c(
                "Línea base" = "Línea base",
                "Contención temprana" = "Contención temprana",
                "Contención tardía" = "Contención tardía",
                "Alta movilidad" = "Alta movilidad",
                "Presión de variantes" = "Presión de variantes",
                "Análisis de sensibilidad" = "Análisis de sensibilidad",
                "Ejemplo docente" = "Ejemplo docente"
              ),
              selected = "Línea base"
            ),
            div(class = "scenario-action-row",
              actionButton("save_current_scenario", "Guardar escenario", icon = icon("save"), class = "btn btn-primary"),
              actionButton("duplicate_selected_scenario", "Duplicar seleccionado", icon = icon("copy"), class = "btn btn-default"),
              actionButton("restore_selected_scenario", "Restaurar configuración", icon = icon("rotate-left"), class = "btn btn-default"),
              actionButton("delete_selected_scenario", "Eliminar seleccionado", icon = icon("trash"), class = "btn btn-default")
            ),
            br(),
            selectInput("selected_saved_scenario", "Escenario guardado:", choices = character(0)),
            div(class = "scenario-action-row",
              downloadButton("download_scenario_card_html", "Exportar tarjeta HTML"),
              downloadButton("download_scenario_config_json", "Exportar JSON"),
              downloadButton("download_scenario_summary_csv", "Exportar tabla CSV"),
              actionButton("compare_saved_scenario", "Comparar con actual", icon = icon("chart-line"), class = "btn btn-default")
            ),
            br(),
            fileInput("import_scenario_config_json", "Import JSON configuration:", accept = c(".json", "application/json")),
            actionButton("import_scenario_config", "Cargar configuración importada", icon = icon("upload"), class = "btn btn-default")
          ),
          div(class = "scenario-lab-box",
            h4("Propuestas de exploración"),
            tags$ul(
              tags$li("¿Puedes retrasar el pico activo sin aumentar los fallecimientos finales?"),
              tags$li("¿Qué cambia más cuando la contención empieza tarde: el momento del pico o la mortalidad final?"),
              tags$li("¿Qué países iniciales aceleran más la dispersión global con la misma semilla?"),
              tags$li("¿Cuánto cambia la curva el compartimento expuesto frente a SIRD?"),
              tags$li("¿Una mayor presión de reemplazo cambia una ejecución concreta o la distribución entre semillas?")
            )
          ),
          fluidRow(
            column(5,
              h4("Tarjeta del escenario seleccionado"),
              uiOutput("selected_scenario_card")
            ),
            column(7,
              h4("Comparación A/B/C"),
              tableOutput("scenario_comparison_table")
            )
          ),
          hr(),
          h4("Curvas comparadas"),
          div(class = "scenario-action-row",
            downloadButton("download_scenario_comparison_csv", "Exportar comparación CSV"),
            downloadButton("download_scenario_comparison_html", "Exportar comparación HTML")
          ),
          plotlyOutput("scenario_comparison_plot", height = "420px"),
          hr(),
          h4("Biblioteca de escenarios"),
          tableOutput("scenario_library_table")
        ),

        tabPanel("Dinámica de variantes",
        ...
        tabPanel("Dispersión por país",
          h4("Distribución de infecciones por país"),
          plotlyOutput("country_distribution", height = "420px"),
          hr(),
          h4("Países alcanzados a lo largo del tiempo"),
          plotlyOutput("country_spread_timeline", height = "320px"),
          hr(),
          h4("Países más afectados"),
          tableOutput("top_countries_table"),
          conditionalPanel(
            condition = "input.active_scenario_mode == 'complete'",
            hr(),
            h4("Principales corredores internacionales de pasajeros usados por el modelo"),
            tableOutput("top_passenger_routes_table"),
            hr(),
            h4("Diagnósticos de dispersión y representación"),
        ...
        tabPanel("Ajuste por edad y coste",
          conditionalPanel(
            condition = "input.active_scenario_mode == 'complete'",
            h4("Parámetros por grupo de edad usados por el modelo"),
            tableOutput("age_parameter_table"),
            hr()
          ),
          h4("Resumen por edad tras la simulación"),
          ...
          conditionalPanel(
            ...
            hr(),
            h4("Informe de evaluación para copiar y pegar"),
            verbatimTextOutput("evaluation_report")
          )
        ),

        tabPanel("Calibración y representatividad",
        ...
        tabPanel("Medidas de contención",
        ...
        tabPanel("Lógica del simulador",
          h4("How the simulator works"),
          tags$p(class = "small-note", "Este diagrama resume la lógica de la herramienta y destaca qué está visible en modo básico guiado y en modo avanzado completo."),
          div(class = "logic-box both",
            h4("1. Comparador COVID"),
          ...
          div(class = "logic-box guided",
            ...
            tags$p("El usuario define la simulación configurable. En modo básico guiado solo permanecen visibles los controles más claros. En modo avanzado completo siguen disponibles todos los controles técnicos."),
            fluidRow(
              column(6,
                tags$b("Visible in basic mode"),
                tags$ul(
                  tags$li("País inicial"),
                  tags$li("Capacidad inicial de expansión"),
                  tags$li("Active case duration"),
                  tags$li("Mortalidad media (%)"),
        ...
        tabPanel("Alcance, supuestos y limitaciones",
          h4("Qué hace esta herramienta"),
          tags$ul(
          ...
          h4("Qué no hace esta herramienta"),
          tags$ul(
          ...
          hr(),
          h4("Checklist de despliegue y estabilidad"),
          tableOutput("deployment_checklist_table"),
          hr(),
          h4("Registro de supuestos por defecto"),
          tableOutput("default_assumptions_registry"),
          hr(),
          h4("Fuentes bibliográficas de los valores por defecto actuales"),
          ...
          hr(),
          h4("Supuestos SEIRD y del compartimento expuesto"),
          ...
          hr(),
          h4("Supuestos del módulo dinámico"),
          ...
          hr(),
          h4("Metadatos del comparador"),
          tableOutput("reference_comparator_metadata_table"),
          hr(),
          h4("Informe de alcance para copiar y pegar"),
          verbatimTextOutput("model_scope_report")
        ),

        tabPanel("Análisis de R0",
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    hidden_in_guided <- c("Calibración y representatividad", "Medidas de contención")
    if (!advanced && !is.null(input$main_tabs) && input$main_tabs %in% hidden_in_guided) {
      updateTabsetPanel(session, "main_tabs", selected = "Mapa mundial")
    }
    if (!dynamic_on && !is.null(input$main_tabs) && identical(input$main_tabs, "Dinámica de variantes")) {
      updateTabsetPanel(session, "main_tabs", selected = "Mapa mundial")
    }
  })
  ...
  capture_current_scenario_config <- function() {
    list(
      app_file = "app_epidemiologic_v16_scenario_lab_step6_presets_guided_exploration_ES_fix6.R",
      saved_at = as.character(Sys.time()),
      scenario_tag = if (!is.null(input$scenario_tag)) input$scenario_tag else "Línea base",
  ...
  scenario_summary_from_results <- function(name, config, results) {
    ...
    data.frame(
      Name = name,
      Label = if (!is.null(config$scenario_tag)) config$scenario_tag else "Línea base",
  ...
  build_scenario_interpretation <- function(summary_row, comparator_row = NULL) {
    ...
    if (!is.null(comparator_row) && nrow(comparator_row) > 0) {
      ...
      timing <- ifelse(delta_day > 0, paste0("más tarde (+", delta_day, " days)"), ifelse(delta_day < 0, paste0("antes (", delta_day, " days)"), "el mismo día"))
      peak_txt <- ifelse(delta_peak > 0, paste0("mayor (+", format(round(delta_peak), big.mark = ","), ")"), ifelse(delta_peak < 0, paste0("menor (", format(round(delta_peak), big.mark = ","), ")"), "similar"))
      ...
      return(paste(base, "Compared with the comparator, the peak occurs", timing, ", el pico activo es", peak_txt, "and the simulation ends with", death_txt, "."))
    }
    base
  }
  ...
  current_comparison_rows <- reactive({
    ...
    if (!is.null(sim_data$hantavirus$data)) {
      ...
      rows[["actual"]] <- scenario_summary_from_results("Simulación actual", cfg, sim_data$hantavirus)
    }
  ...
  dynamic_preset_values <- function(preset) {
    ...
    if (identical(preset, "off")) {
      return(list(enabled = FALSE, scenario = "off", scenario_label = "Desactivado", rate = 0, targets = 30, calibration = "conservative", max_multiplier = 1.0, adaptive_saturation = FALSE, saturation_exponent = 1.0, interpretation = "Off: no dynamic replacement module."))
    }
  ...
  get_dynamic_config <- function() {
    ...
    list(
      ...
      scenario_label = if (enabled) "Custom advanced" else "Desactivado",
  ...
  output$covid_reference_card <- renderUI({
    ...
    comparator_items <- list(
      c("Tipo", dplyr::case_when(
        identical(ref_type, "age_adjusted_seird") ~ "Omicron ajustado por edad SEIRD RDS",
        identical(ref_type, "age_adjusted_sird") ~ "Omicron ajustado por edad SIRD RDS",
        identical(ref_type, "age_adjusted") ~ "Omicron ajustado por edad SIRD RDS",
        TRUE ~ "Omicron básico SIRD RDS"
      )),
      c("Modelo", ref_model),
      c("E: fase expuesta", ifelse(identical(ref_model, "SEIRD"), paste0(ref_exposed_period, " days to active phase"), "No aplica en SIRD")),
      c("Pico E", ifelse(is.na(ref_peak_exposed), "No disponible", paste0(ref_peak_exposed, " persons; day ", ref_peak_exposed_day))),
      c("Referencia", "COVID-19 Omicron"),
      c("País inicial", "South Africa"),
      c("R0", "4.25"),
      c("Ventana activa", "5 days"),
      c("Motor", ifelse(grepl("age_adjusted", ref_type), "CFR diferencial ajustado por edad", "Referencia agregada básica"))
    )
  ...
  output$deployment_checklist_table <- renderTable({
    data.frame(
      Item = c(
        ...
        "Persistencia del laboratorio de escenarios"
      ),
      Expected = c(
  ...
  output$reference_comparator_metadata_table <- renderTable({
    ...
    data.frame(
      ...
      Value = c(
        ...
        ref_model,
        ifelse(identical(ref_model, "SEIRD"), paste0("Included; ", ref_exposed, " days from exposure to active phase"), "No aplica en SIRD"),
        "South Africa",
        "4.25",
        "5 days",
  ...
  output$evaluation_report <- renderText({
    ...
    lines <- c(
      ...
      paste0("app_file=app_epidemiologic_v16_scenario_lab_step6_presets_guided_exploration_ES_fix6.R"),
      paste0("active_scenario_mode=", isolate(input$active_scenario_mode)),
  ...
  output$assumptions_table <- renderTable({
    data.frame(
      Component = c(
        "Dinámica por país", "Estructura por edad de infección", "Tráfico de pasajeros", "Dispersión internacional", "Establecimiento importado", "Diagnósticos de representación", "Estado basal", "Aparición de variantes", "Reemplazo de variantes", "Comparador COVID-19 Omicron"
      ),
      Assumption = c(
        "Cada país tiene compartimentos S, E, I, R y D separados cuando SEIRD está activo; SIRD permanece disponible para análisis de sensibilidad.",
        "Las infecciones se almacenan por días desde la infección. Un kernel de infectividad normalizado, gaussiano o plano, determina la presión infecciosa actual.",
        "La difusión usa una matriz bilateral explícita de flujo anual de pasajeros y un pequeño componente residual de tráfico aéreo para rutas no listadas.",
        "Los viajeros infecciosos esperados se calculan a partir del flujo de pasajeros y de la prevalencia por edad de infección en el país de origen.",
        "La probabilidad de que un viajero origine transmisión local depende del Rt de destino y de la infectividad restante, con un modificador de oportunidad visible para el usuario.",
        "Las tablas diagnósticas comparan importaciones internas, country_history, umbrales de visualización y elegibilidad del mapa para identificar fallos de representación.",
        "En el día 0 solo está infectado el país inicial seleccionado. Los demás países solo pueden recibir infecciones importadas cuando avanza la simulación.",
        "Las macrovariantes candidatas dependen de infecciones incidentes, tasa dinámica, oportunidades efectivas relevantes para la transmisión y nivel de calibración.",
        "Solo las variantes beneficiosas establecidas entran en el conjunto circulante; sus frecuencias evolucionan según R0 efectivo relativo, con límite para evitar valores no realistas.",
        "La referencia COVID-19 Omicron empieza en Sudáfrica, usa parámetros tipo Omicron, desactiva evolución posterior de variantes y se carga desde un archivo RDS en lugar de recalcularse tras cada simulación del usuario."
      ),
      stringsAsFactors = FALSE
    )
  }, rownames = FALSE)
  ...
  observeEvent(input$run_simulation, {
    ...
    exposed_days <- if (isTRUE(guided_mode)) 4 else if (!is.null(input$exposed_period_days)) input$exposed_period_days else 4
  ...
  output$plot_infected <- renderPlotly({
    ...
    p <- plot_ly() %>%
      ...
      layout(title = "Población activa infectada a lo largo del tiempo", xaxis = list(title = "Days"), yaxis = list(title = global_axis_title("Active infected")), legend = list(orientation = "h", x = 0, y = 1.12))
  ...
  output$plot_deaths <- renderPlotly({
    ...
    p <- plot_ly() %>%
      ...
      layout(title = "Fallecimientos acumulados a lo largo del tiempo", xaxis = list(title = "Days"), yaxis = list(title = global_axis_title("Cumulative deaths")), showlegend = FALSE)
  ...
  output$plot_recovered <- renderPlotly({
    ...
    p <- plot_ly() %>%
      ...
      layout(title = "Recuperados acumulados a lo largo del tiempo", xaxis = list(title = "Days"), yaxis = list(title = global_axis_title("Cumulative recovered")), showlegend = FALSE)
    p %>% config(displayModeBar = TRUE)
  })
  ...
  output$global_peak_summary_table <- renderTable({
    ...
    data.frame(
      Series = c("Nuevo virus", "Omicron reference"),
  ...
  output$connectivity_preview_map <- renderLeaflet({
    ...
    legend_html <- HTML(paste0(
      ...
      "Ancho de línea = peso relativo de pasajeros",
      "</div>"
    ))
  ...
  observeEvent(input$save_current_scenario, {
    ...
    showNotification(paste("Escenario guardado:", nm), type = "message", duration = 3)
  })
  ...
  output$selected_scenario_card <- renderUI({
    ...
    if (is.null(item) || is.null(item$summary)) {
      return(tags$div(class = "scenario-card-polished", tags$p("No hay escenario seleccionado.")))
    }
    s <- item$summary
    comparator <- NULL
    if (!is.null(sim_data$covid$data)) {
      covid_like <- list(
        ...
        compartment_model = if (!is.null(sim_data$covid$compartment_model)) sim_data$covid$compartment_model else "SIRD",
        exposed_period_days = if (!is.null(sim_data$covid$exposed_period_days)) sim_data$covid$exposed_period_days else NA,
        cost_profile = list(rds_reference = if (!is.null(sim_data$covid$reference_file)) sim_data$covid$reference_file else "Comparador COVID", comparator_structure_warning = "reference")
      )
      ...
      comparator <- scenario_summary_from_results("Comparador COVID", cfg, covid_like)
    }
    tags$div(class = "scenario-card-polished",
      ...
      div(class = "scenario-subtitle",
        ...
        tags$span(class = "scenario-tag", paste("Inicio:", s$StartCountry))
      ),
      tags$div(class = "scenario-kpi-grid",
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Día del pico"), tags$div(class = "scenario-kpi-value", s$PeakDay)),
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Pico activo"), tags$div(class = "scenario-kpi-value", format(round(s$PeakActive), big.mark = ","))),
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Pico expuesto"), tags$div(class = "scenario-kpi-value", ifelse(is.na(s$PeakExposed), "NA", format(round(s$PeakExposed), big.mark = ",")))),
        tags$div(class = "scenario-kpi", tags$div(class = "scenario-kpi-label", "Fallecimientos finales"), tags$div(class = "scenario-kpi-value", format(round(s$FinalDeaths), big.mark = ",")))
      ),
  ...
  output$scenario_library_table <- renderTable({
    ...
    if (nrow(df) == 0) return(data.frame(Message = "No hay escenarios guardados."))
    df[, c("Name", "Label", "Modelo", "StartCountry", "Days", "R0", "PeakDay", "PeakActive", "FinalDeaths", "CountriesReached", "Comparator", "StructureWarning"), drop = FALSE]
  }, digits = 2)

  output$scenario_comparison_table <- renderTable({
    out <- current_comparison_rows()
    if (nrow(out) == 0) return(data.frame(Message = "Ejecuta o guarda un escenario para comparar."))
    keep <- intersect(c("Name", "Label", "Modelo", "StartCountry", "PeakDay", "PeakActive", "PeakActivePercentWorld", "PeakExposedDay", "PeakExposed", "FinalDeaths", "CountriesReached", "DeltaPeakDayVsComparator", "DeltaPeakActiveVsComparator", "DeltaPeakActivePercentVsComparator", "DeltaFinalDeathsVsComparator", "DeltaFinalDeathsPercentVsComparator"), names(out))
    out[, keep, drop = FALSE]
  }, digits = 2)
  ...
  output$scenario_comparison_plot <- renderPlotly({
    ...
    p <- plot_ly()
    if (!is.null(sim_data$hantavirus$data)) {
      p <- p %>% add_lines(data = sim_data$hantavirus$data, x = ~time, y = ~I, name = "Simulación actual", line = list(width = 3))
    }
    compare_id <- scenario_library$compare_id
    if (!is.null(compare_id) && !is.null(scenario_library$items[[compare_id]])) {
  ...
  output$download_scenario_comparison_csv <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (nrow(df) == 0) df <- data.frame(Message = "No hay comparación disponible")
      write.csv(df, file, row.names = FALSE)
    }
  )
  ...
  output$download_scenario_comparison_html <- downloadHandler(
    ...
    content = function(file) {
      df <- current_comparison_rows()
      if (nrow(df) == 0) df <- data.frame(Message = "No hay comparación disponible")
      keep <- intersect(c("Name", "Modelo", "StartCountry", "PeakDay", "PeakActive", "FinalDeaths", "CountriesReached", "DeltaPeakDayVsComparator", "DeltaPeakActiveVsComparator", "DeltaFinalDeathsVsComparator"), names(df))
      ...
      html <- paste0(
        "<!doctype html><html><head><meta charset='utf-8'><title>Comparación de escenarios</title>",
        ...
        "</head><body><h1>Comparación de escenarios</h1><table>", headers, paste(rows, collapse = ""), "</table></body></html>"
      )
      writeLines(html, file, useBytes = TRUE)
    }
  )
