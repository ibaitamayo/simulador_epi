...
server <- function(input, output, session) {
  ...
  capture_current_scenario_config <- function() {
    list(
      app_file = "app_epidemiologic_v16_scenario_lab_step4b_comparator_description.R",
  ...
  output$evaluation_report <- renderText({
    ...
    lines <- c(
      ...
      paste0("app_file=app_epidemiologic_v16_scenario_lab_step4b_comparator_description.R"),
      paste0("active_scenario_mode=", isolate(input$active_scenario_mode)),
